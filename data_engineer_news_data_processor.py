...
class KeywordBasedClassifier:
    ...
    def __init__(self):
        self.keyword_rules = {
            "cloud_dw": ["AWS", "GCP", "Azure", "Snowflake", "Databricks", "Redshift", "BigQuery"],
            "etl_orchestration": ["Airflow", "Spark", "Flink", "Beam", "Dagster", "Prefect"],
            "db_storage": ["MySQL", "PostgreSQL", "Oracle", "MongoDB", "Redis", "DuckDB", "ClickHouse"],
            "infra_devops": ["Kubernetes", "Docker", "Terraform", "Ansible", "Helm", "Prometheus", "Grafana"],
            "ml_ai_integration": ["LangChain", "Vector DB", "Chroma", "Weaviate", "Pinecone", "MLflow"],
            "trends_governance": ["Data Mesh", "DataOps", "Observability", "Governance", "GDPR", "규제", "보안"],
        }
...
class OptimizedNewsClassifier:
    ...
    def __init__(self, api_key: str):
        ...
        # 뉴스 카테고리 정의
        self.categories = {
            "cloud_dw": "Cloud/Data Warehouse",
            "etl_orchestration": "ETL & Orchestration",
            "db_storage": "Database & Storage",
            "infra_devops": "Infra/DevOps",
            "ml_ai_integration": "ML & AI Integration",
            "trends_governance": "Trends & Governance",
            "other": "Other News"
        }
    ...
    def _classify_batch_with_llm(self, titles: List[str]) -> Dict[str, List[Dict]]:
        ...
        prompt = f"""다음 데이터 엔지니어링 관련 뉴스 헤드라인들을 카테고리에 맞게 분류해주세요.

        카테고리 설명:
        - cloud_dw: AWS, GCP, Azure, Snowflake, Databricks 등 클라우드 및 데이터웨어하우스
        - etl_orchestration: Apache Spark, Flink, Airflow, Prefect 등 ETL/오케스트레이션 도구
        - db_storage: MySQL, PostgreSQL, Redis, DuckDB, ClickHouse 등 데이터베이스/스토리지
        - infra_devops: Kubernetes, Docker, Terraform, Observability, 모니터링/배포 자동화
        - ml_ai_integration: Vector DB, MLflow, LangChain, AI와의 연계/통합
        - trends_governance: Data Mesh, DataOps, 보안, 규제, 데이터 거버넌스 및 업계 트렌드
        - other: 위에 해당하지 않는 뉴스

        JSON 형식으로만 응답:
        {{"cloud_dw": [{{"index": 0, "title": "제목"}}], "etl_orchestration": [], ...}}
        """
...
class OptimizedNewsProcessor:
    ...
    def get_todays_news(self) -> List[Dict]:
        ...
        try:
            response = self.supabase.table('data_engineer_news') \
                .select('*') \
    ...
    def save_to_newsletter_sections(self, classified_results: Dict[str, Dict]) -> None:
        ...
        try:
            ...
            if sections_to_insert:
                ...
                self.supabase.table('data_engineer_news') \
